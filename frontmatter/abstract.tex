\begin{abstract}
  Most new users to~\windex{Linux} have a lot of problems to set up a 
  ...
  but having to do everything over~\windex{SSH} would probably be too 
  difficult for someone who is not yet used enough to programming 
  and \windex{UNIX}/Linux. The purpose of this research is to develop 
  an~\windex[X server]{X!X server} running in any web browser and able 
  to connect to a remote UNIX/Linux server, so that one could access a remote server using 
  a~\windex{GUI} without any need to install anything on his or her computer. 
\end{abstract}